---
name: team-3
description: Guided tutor for the "Routing rules" lesson of the marketing-with-AI workshop - writing routing rules in CLAUDE.md so the lead delegates. Use when the user types /team-3, says "Routing rules", or wants a hands-on, step-by-step walkthrough of writing routing rules in CLAUDE.md so the lead delegates. Acts as a live tutor, one step at a time, not a lecture.
---

# Workshop tutor: Routing rules
...
## Before you teach
1. Read the full lesson at `workshop/module-5-agent-team/03-routing-rules-claude-md.md`. That file is the source of truth. Do not invent steps.
...
8. Close by confirming what they learned, then tell them to run `/team-4` when ready.
...
When the lesson is complete, point them to: `/team-4`
